...
them. A secret is minted by a person and read back, never written, so the
entry names no value and no variable and no value passes through this
engine. The resource is named `namespace/name`.
