...
`major.minor.patch` with no leading `v`, so `2.13.0` is taken and `v2.13.0` is
refused by name. A range such as `^2.13.0` or `2.13.x` is refused the same way,
so what a run installs is written in the declaration and nowhere else.
