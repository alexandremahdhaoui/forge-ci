...
The chart comes from `spec.repository` and `spec.version` and from nothing
else. The manager fetches it into a scratch directory it makes for the life
of one install and removes when the install returns. No repository cache, no
repository file, no registry configuration and no downloader plugin named by
an environment variable reaches that fetch, so no environment variable
decides which bytes a chart is made of. An `https://` repository is read with
no credential of any kind, so a private one over https is out of reach. An
`oci://` repository is still read through the container credential store in
the operator's home, which the declaration does not name.
