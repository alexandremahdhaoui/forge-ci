...
wrote `spec.mint`, the error adds that the pipeline file holds the minting
steps under `spec.mint` of the secret entry naming that namespace and name.
The text itself never travels into an error, because a run log is read by
everyone who reads the run and the entry is where an operator reads it. The
manager names no key type and no product of its own, because what a secret is
belongs to the declaration that asked for it.
