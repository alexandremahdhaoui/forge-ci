...
import (
	...
	"github.com/alexandremahdhaoui/forge-ci/internal/adapter/terraformadapter"
...
func NewHandlers() Handlers {
	...
	return Handlers{
		Reconcile: func(ctx context.Context, in ReconcileInput) (*ReconcileOutput, error) {
			root, err := terraformadapter.New()
			if err != nil {
				return nil, fmt.Errorf("building the terraform client of manager %s: %w", in.Manager, err)
			}

			ctrl := managercontroller.New(managercontroller.NewTerraformRealizer(ctx, root), fs)
