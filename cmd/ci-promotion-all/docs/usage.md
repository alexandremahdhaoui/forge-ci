...
A promotion is the decision to move on, taken once per stage over every substage
outcome. It is a port, so `all`, a threshold, and anything you write yourself are
all just engines.
...
A substage counts as passed only when its status is `passed` **and** every gate
passed. A pending gate blocks the stage.
