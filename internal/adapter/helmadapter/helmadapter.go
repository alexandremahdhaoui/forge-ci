...
func (r Releases) InstallRelease(ctx context.Context, declared citypes.HelmRelease) error {
	cfg, err := r.open(declared.Namespace)
	if err != nil {
		return err
	}
...
func absent(charter chart.Charter) bool {
	held := reflect.ValueOf(charter)

	return !held.IsValid() || (held.Kind() == reflect.Pointer && held.IsNil())
}
