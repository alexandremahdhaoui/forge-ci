...
func TestAShortNameResolvesToOurOwnModuleAtOurOwnVersion(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t,
		[]string{"run", "github.com/alexandremahdhaoui/forge-ci/cmd/ci-promotion-all@v0.2.0"},
		cmd.Args)
}
...
func TestADirtyOwnVersionIsStrippedForGoRun(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t,
		[]string{"run", "github.com/alexandremahdhaoui/forge-ci/cmd/ci-state-git@v0.2.0"},
		cmd.Args)
}
