...
func TestAnInstalledBinaryWins(t *testing.T) {
	...
	cmd, err := r.Resolve("go://github.com/x/forge-ci/cmd/ci-compute-local@v0.1.0")
...
func TestTheSourceTreeIsUsedWhenNothingIsInstalled(t *testing.T) {
	...
	cmd, err := r.Resolve("go://github.com/x/forge-ci/cmd/ci-state-git@v0.1.0")
	require.NoError(t, err)
...
func TestAModuleFallsBackToGoRun(t *testing.T) {
	...
	cmd, err := r.Resolve("go://github.com/x/forge-ci/cmd/ci-gate-manual@v0.2.0")
	require.NoError(t, err)
	require.Equal(t, "go", cmd.Path)
...
func TestAShortNameResolvesToOurOwnModule(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t,
		[]string{"run", "github.com/alexandremahdhaoui/forge-ci/cmd/ci-promotion-all@latest"},
		cmd.Args)
}
