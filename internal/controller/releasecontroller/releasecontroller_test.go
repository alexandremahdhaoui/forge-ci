...
func TestItTagsAndPublishes(t *testing.T) {
	...
	git.EXPECT().TagAt(context.Background(), dir, tag).Return("", false, nil).Once()
...
func TestItPublishesATagThatAlreadyExists(t *testing.T) {
	...
	git := gitadaptermock.NewMockGit(t)
	git.EXPECT().TagAt(context.Background(), dir, tag).Return(sha, true, nil).Once()
...
func TestItRefusesToRepointATag(t *testing.T) {
	...
	git := gitadaptermock.NewMockGit(t)
	git.EXPECT().TagAt(context.Background(), dir, tag).Return("999999", true, nil).Once()
...
func TestItReportsAFailureFromEitherHalf(t *testing.T) {
	...
	reading := gitadaptermock.NewMockGit(t)
	reading.EXPECT().TagAt(context.Background(), dir, tag).Return("", false, errBoom).Once()

	_, err := releasecontroller.New(reading, githubadaptermock.NewMockAPI(t)).
		Publish(context.Background(), dir, repo, tag, sha)
	require.ErrorIs(t, err, errBoom)

	tagging := gitadaptermock.NewMockGit(t)
	tagging.EXPECT().TagAt(context.Background(), dir, tag).Return("", false, nil).Once()
	...
	git2.EXPECT().TagAt(context.Background(), dir, tag).Return(sha, true, nil).Once()
